...
export function useInteractionState(options: UseInteractionStateOptions = {}) {
  const { defaultDisabled = false, defaultReason, defaultTooltip } = options;
  ...
  const disable = useCallback(
    (
      reason?: string,
      tooltip?: string,
      visualCue?: InteractionState['visualCue']
    ) => {
      setState({
        isDisabled: true,
        reason,
  ...
  const setLoadingState = useCallback(
    (isLoading: boolean, reason?: string) => {
      if (isLoading) {
        disable(
          reason || 'Loading...',
          reason || 'Please wait while the operation completes',
          'loading'
        );
      } else {
        enable();
