...
describe('Breadcrumb Navigation Properties', () => {
  ...
  describe('Property 33: Multi-step processes display breadcrumb navigation', () => {
        ...
        fc.property(
          ...
          (items: BreadcrumbItem[]) => {
            ...
            const { container, unmount } = render(
              <Breadcrumb items={processedItems} showHome={false} />
            );
            ...
            try {
              ...
              processedItems.forEach((item) => {
                ...
                ).filter((el) => el.textContent?.trim() === item.label);
              ...
              const linkItems = processedItems.filter(
                (item) => item.href && !item.active
              );
    ...
    it('should display process-specific breadcrumbs for upload workflow', () => {
      fc.assert(
        fc.property(fc.integer({ min: 1, max: 4 }), (currentStep: number) => {
          const { container, unmount } = render(
            <UploadProcessBreadcrumb currentStep={currentStep} />
          );
    ...
    it('should display process-specific breadcrumbs for payment workflow', () => {
      fc.assert(
        fc.property(fc.integer({ min: 1, max: 3 }), (currentStep: number) => {
          const { container, unmount } = render(
            <PaymentProcessBreadcrumb currentStep={currentStep} />
          );
      ...
      fc.assert(
        fc.property(
          ...
          (items: BreadcrumbItem[]) => {
            ...
            const { container, unmount } = render(
              <Breadcrumb items={processedItems} showHome={true} />
            );
