  ...
  return lazy(() => {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        Component.displayName = `LargeComponent${name}`;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ componentName, loadingText }) => {
          const componentSize =
            mockBundleAnalyzer.getComponentSize(componentName);
          const shouldBeLazyLoaded =
            mockBundleAnalyzer.shouldLazyLoad(componentName);
          ...
          if (shouldBeLazyLoaded) {
            ...
            TestComponent = createMockLargeComponent(
              componentName,
              componentSize
            );
          } else {
            ...
            RegularComponent.displayName = `RegularComponent${componentName}`;
          ...
          if (shouldBeLazyLoaded) {
            ...
            try {
              ...
              expect(
                container.querySelector('[data-testid="loading-fallback"]')
              ).toBeInTheDocument();
                ...
                () => {
                  expect(
                    container.querySelector(
                      `[data-testid="large-component-${componentName}"]`
                    )
                  ).toBeInTheDocument();
                },
                { timeout: 1000 }
              ...
              expect(container.textContent).toContain(
                `Large Component ${componentName} (Size: ${componentSize}KB)`
              );
            } finally {
              unmount();
            }
      ...
      fc.property(
        ...
        ({ fallbackType, fallbackText }) => {
          ...
          try {
            ...
            expect(
              container.querySelector(
                `[data-testid="${fallbackType}-fallback"]`
              )
            ).toBeInTheDocument();
