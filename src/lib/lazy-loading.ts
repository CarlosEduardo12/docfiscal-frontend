...
export function shouldLazyLoadComponent(componentName: string): boolean {
  const size =
    COMPONENT_SIZES[componentName as keyof typeof COMPONENT_SIZES] || 50;
...
export const LazyComponents = {
  ...
  OrderHistoryTable: createLazyComponent(
    () =>
      import('@/components/order/OrderHistoryTable').then((module) => ({
        default: module.OrderHistoryTable,
      })),
    'OrderHistoryTable'
  ),

  UploadArea: createLazyComponent(
    () =>
      import('@/components/upload/UploadArea').then((module) => ({
...
export class LazyLoadingMetrics {
  ...
  static recordComponentLoad(componentName: string, loadTime: number) {
    ...
    const size =
      COMPONENT_SIZES[componentName as keyof typeof COMPONENT_SIZES] || 0;
    this.metrics.lazyLoadedSize += size;

    if (process.env.NODE_ENV === 'development') {
      console.log(
        `Lazy loaded component: ${componentName} (${size}KB) in ${loadTime}ms`
      );
    }
  }
