...
export class AssetOptimizer {
  ...
  private static calculateOptimization(asset: AssetInfo): OptimizationResult {
    ...
    const compressionRatio =
      ASSET_CONFIG.COMPRESSION_RATIOS[
    ...
    if (asset.priority === 'critical' || asset.priority === 'high') {
      ...
      asset.position === 'above-fold' &&
      sizeInKB <= ASSET_CONFIG.LAZY_LOAD_THRESHOLD
    ) {
...
export class AssetLazyLoadManager {
   ...
   */
  private static async loadVideo(
...
export class AssetPerformanceMonitor {
  ...
  static getMetrics() {
    return {
      ...this.metrics,
      successRate:
        this.metrics.totalAssets > 0
          ? this.metrics.loadedAssets / this.metrics.totalAssets
          : 0,
      averageLoadTime:
        this.metrics.loadedAssets > 0
