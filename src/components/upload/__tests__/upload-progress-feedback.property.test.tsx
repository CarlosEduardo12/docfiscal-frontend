        ...
        fc.property(
          ...
          (progressData) => {
            ...
            if (
              ...
              expect(
                screen.getByText(`${progressData.progress}%`)
              ).toBeInTheDocument();
            }
            ...
            if (progressData.status === 'completed') {
              expect(
                screen.getByText(/concluído|completo|finalizado/i)
              ).toBeInTheDocument();
        ...
        fc.property(
          ...
          (progressData) => {
            ...
            expect(progressBar).toHaveAttribute(
              'aria-valuenow',
              progressData.progress.toString()
            );
        ...
        fc.property(
          fc.record({
            ...
            fileName: fc
              ...
              .map(
                (s) =>
                  `status-test-${s.trim().replace(/[^a-zA-Z0-9]/g, 'x')}.pdf`
              ),
          }),
          (statusData) => {
            const props = {
            ...
            switch (statusData.status) {
              case 'uploading':
                expect(
                  container.querySelector('[class*="text-blue-600"]')
                ).toHaveTextContent(/enviando|carregando|upload/i);
                break;

              case 'processing':
                expect(
                  container.querySelector('[class*="text-blue-600"]')
                ).toHaveTextContent(/processando|convertendo/i);
                break;

              case 'completed':
                expect(
                  container.querySelector('[class*="text-green-600"]')
                ).toHaveTextContent(/concluído|completo|finalizado|sucesso/i);
                break;

              case 'error':
                expect(
                  container.querySelector('[class*="text-red-600"]')
                ).toHaveTextContent(/erro|falha/i);
                break;
            }
        ...
        fc.property(
          fc.record({
            ...
            fileName: fc
              ...
              .map(
                (s) =>
                  `action-test-${s.trim().replace(/[^a-zA-Z0-9]/g, 'x')}.pdf`
              ),
          ...
          (actionData) => {
            ...
            if (
              actionData.hasCancel &&
              (actionData.status === 'uploading' ||
        ...
        fc.property(
          fc.record({
            ...
            fileName: fc
              ...
              .filter((s) => s.trim().length > 0)
              .map(
                (s) => `size-test-${s.trim().replace(/[^a-zA-Z0-9]/g, 'x')}.pdf`
              ),
          }),
          (formatData) => {
            render(
