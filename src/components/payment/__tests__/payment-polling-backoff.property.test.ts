    ...
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          ...
          backoffMultiplier: fc
            .float({ min: Math.fround(1.1), max: Math.fround(3.0) })
        ...
        async (testData) => {
          ...
          for (let attempt = 0; attempt < testData.maxAttempts; attempt++) {
            ...
            if (attempt > 0) {
              expect(currentInterval).toBeGreaterThanOrEqual(
                pollIntervals[attempt - 1]
              );
            }
            ...
            currentInterval = Math.min(
              isNaN(nextInterval) || !isFinite(nextInterval)
                ? currentInterval * 1.5
                : nextInterval,
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          ...
          errorBackoffMultiplier: fc
            .float({ min: Math.fround(2.0), max: Math.fround(5.0) })
            .filter((n) => !isNaN(n)),
        }),
        async (testData) => {
          ...
          for (let attempt = 0; attempt < 10; attempt++) {
            ...
            if (attempt < testData.errorCount) {
              ...
              const nextInterval =
                currentInterval * testData.errorBackoffMultiplier;
          ...
          if (testData.errorCount > 1) {
            ...
            const expectedMinInterval =
              testData.baseInterval *
              Math.pow(
                testData.errorBackoffMultiplier,
                testData.errorCount - 1
              );
            expect(finalInterval).toBeGreaterThanOrEqual(
              expectedMinInterval * 0.9
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          ...
          backoffFactor: fc
            .float({ min: Math.fround(1.2), max: Math.fround(2.5) })
            .filter((n) => !isNaN(n) && isFinite(n)),
        }),
        async (testData) => {
          const intervals: number[] = [];
          ...
          const allAtMax = finalIntervals.every(
            (interval) => interval === testData.maxInterval
          );
          if (
            testData.maxInterval / testData.initialInterval <
            Math.pow(testData.backoffFactor, 7)
          ) {
